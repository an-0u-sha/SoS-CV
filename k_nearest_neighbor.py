...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        num_train = self.X_train.shape[0]
        dists = np.zeros((num_test, num_train))
        test_squared = np.sum(X**2, axis=1, keepdims=True)  # shape (num_test, 1)
        train_squared = np.sum(self.X_train**2, axis=1)  # shape (num_train,)
        train_squared = train_squared.reshape(1, -1)  # shape (1, num_train)
        cross_term = X.dot(self.X_train.T)  # shape (num_test, num_train)
        dists = np.sqrt(test_squared - 2 * cross_term + train_squared)
    ...
    def predict_labels(self, dists, k=1):
        ...
        # Find the most common label among closest_y
